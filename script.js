...
function verificarFinDePartida() {
    ...
    if (puntuacionPareja0 >= 100 || puntuacionPareja1 >= 100) {
        ...
        if (estadoJuego.numeroDeJugadores === 4) {
            nombrePareja0 = `${nombreJ1} y ${estadoJuego.nombreCompanero}`; 
            nombrePareja1 = `${estadoJuego.nombreOponenteDerecha} y ${estadoJuego.nombreOponenteIzquierda}`; 
        } else { // 2 Jugadores
            nombrePareja0 = nombreJ1;
            nombrePareja1 = estadoJuego.nombreCompanero; 
        }
...
function iniciarNuevaRonda(esNuevaPartida = false) {
    ...
    actualizarResaltadoTurnoVisual(); 
...
function jugarFicha(fichaOriginal, extremoDelTableroAlQueConecta) {
    ...
    if (indiceFichaEnMano === -1 && estadoJuego.fichasEnTablero.length > 0) { 
        console.error("Error: Ficha no encontrada en la mano del jugador.", fichaOriginal); // Error crítico
    ...
    if (manoActual.length === 0) { 
        ...
        if (estadoJuego.numeroDeJugadores === 2) {
            ...
            if (estadoJuego.turnoJugador === 0) { 
                ...
                nombreGanadorRonda = estadoJuego.nombreCompanero; 
            }
    ...
    if (estadoJuego.juegoIniciado) {
        if (!puedeJugar(estadoJuego.manosJugadores[estadoJuego.turnoJugador], estadoJuego.extremosAbiertos)) {
            ...
            if (estadoJuego.consecutivePasses >= estadoJuego.numeroDeJugadores) { 
                ...
                const oponente2JNombre = estadoJuego.nombreCompanero; 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (vistaGuardada) { 
        ...
        localStorage.setItem('vistaFichas', estadoJuego.vistaFichas); // Guarda el default si no hay nada
    }
    ...
    if (contrasteGuardado === 'true') {
        ...
        if (contrasteIconContainer) contrasteIconContainer.textContent = '🌙'; 
    } else {
        if (contrasteIconContainer) contrasteIconContainer.textContent = '☀️'; 
    }
...
function actualizarResaltadoTurnoVisual() {
    ...
    todosLosContenedoresIds.forEach(id => {
